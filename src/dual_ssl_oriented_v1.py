...
def run_training(exp_args):
    ...
    orient_alpha = exp_args['orient_alpha']
    ignore_orient_loss = exp_args['ignore_orient_loss']
    use_v2 = exp_args['use_v2']
    ...
    ssl_model = models_dual_ssl.DualSSLOrientedModelV1(network=net,
                                                       ...
                                                       num_images_in_batch=b_size, orient_alpha=orient_alpha,
                                                       ignore_orient_loss=ignore_orient_loss, use_v2=use_v2,
...
def show_images_only(exp_args):
    ...
    tb_transform_train = tb_in12_transforms.get_eval_transform(dset="toybox")
    tb_loader_train = get_dataloader(dset="toybox", batch_size=b_size, ssl_type=tb_ssl_type,
                                     transform=tb_transform_train, hypertune=hypertune, workers=workers, shuffle=True)

    in12_transform_train = tb_in12_transforms.get_eval_transform(dset="in12")
    in12_loader_train = get_dataloader(dset="in12", batch_size=b_size//2, ssl_type=in12_ssl_type,
                                       transform=in12_transform_train, hypertune=hypertune, workers=workers,
                                       shuffle=True)

    show_images(loader=tb_loader_train, mean=datasets.TOYBOX_MEAN, std=datasets.TOYBOX_STD, aug=True, n_cols=8)
    show_images(loader=in12_loader_train, mean=datasets.IN12_MEAN, std=datasets.IN12_STD, aug=True, n_cols=8)
...
def main():
    parser = parsers.get_dual_ssl_oriented_v1_parser()
    exp_args = vars(parser.parse_args())
    if exp_args['show_images']:
        show_images_only(exp_args=exp_args)
    else:
