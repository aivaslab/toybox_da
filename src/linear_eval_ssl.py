...
def main():
    ...
    if exp_args['load_path'] != "" and os.path.isdir(exp_args['load_path']):
        ...
        net = networks.ResNet18SSLLE(num_classes=12, backbone_weights=bb_wts, ssl_weights=ssl_wts,
                                     classifier_weights=None)
    else:
        net = networks.ResNet18SSLLE(num_classes=12, pretrained=exp_args['pretrained'])

    for params in net.backbone.parameters():
        params.requires_grad = False

    num_params_trainable, num_params = net.count_trainable_parameters()
    logger.info(f"{num_params_trainable} / {num_params} parameters are trainable...")
