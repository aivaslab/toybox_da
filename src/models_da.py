...
class DANNModel:
    ...
    def train(self, optimizer, scheduler, steps, ep, ep_total, writer: tb.SummaryWriter):
        ...
        for step in range(1, steps + 1):
            ...
            comb_images = torch.concat([src_images, trgt_images], dim=0)
            bb_feats, cl_logits, dom_logits = self.network.forward(comb_images, alfa)
            src_size = src_images.shape[0]
            src_bb_feats, src_cl_logits, src_dom_logits = \
                bb_feats[:src_size], cl_logits[:src_size], dom_logits[:src_size]

            src_loss = src_criterion(src_cl_logits, src_labels)

            dom_labels = torch.cat([torch.zeros(src_size), torch.ones(src_size)]).cuda()
            dom_loss = dom_criterion(dom_logits, dom_labels)
            total_loss = src_loss + dom_loss
            total_loss.backward()
            optimizer.step()
            ...
            if 0 <= step - halfway < 1:
                self.logger.info("Ep: {}/{}  Step: {}/{}  BLR: {:.4f}  CLR: {:.4f}  CE: {:.4f}  DOM: {:.4f}  "
                                 "Tot: {:.4f}  T: {:.2f}s".format(
                                    ep, ep_total, step, steps,
                                    optimizer.param_groups[0]['lr'], optimizer.param_groups[1]['lr'],
                                    ce_loss_total / num_batches, dom_loss_total / num_batches,
                                    comb_loss_total / num_batches, time.time() - start_time))
            if not self.no_save:
                writer.add_scalars(
                    main_tag="training_loss",
        ...
        self.logger.info("Ep: {}/{}  Step: {}/{}  BLR: {:.4f}  CLR: {:.4f}  CE: {:.4f}  DOM: {:.4f}  "
                         "Tot: {:.4f}  T: {:.2f}s".format(
                            ep, ep_total, steps, steps,
                            optimizer.param_groups[0]['lr'], optimizer.param_groups[2]['lr'],
                            ce_loss_total / num_batches, dom_loss_total / num_batches, comb_loss_total / num_batches,
                            time.time() - start_time)
                         )
    ...
    def calc_val_loss(self, loaders, loader_names, ep, steps, writer: tb.SummaryWriter):
        ...
        for idx, loader in enumerate(loaders):
            ...
            for _, (idxs, images, labels) in enumerate(loader):
                ...
                images, labels = images.cuda(), labels.cuda()
                with torch.no_grad():
                    _, logits, _ = self.network.forward(images)
                    loss = criterion(logits, labels)
                    total_loss += loss.item()
            losses.append(total_loss / num_batches)
    ...
    def eval(self, loader, is_toybox):
        """Evaluate the model on the provided dataloader"""
        n_total = 0
        n_correct = 0
        dom_total = 0
        ...
        for _, (idxs, images, labels) in enumerate(loader):
            images, labels = images.cuda(), labels.cuda()
            with torch.no_grad():
                _, logits, dom_logits = self.network.forward(images)
            top, pred = utils.calc_accuracy(output=logits, target=labels, topk=(1,))
            n_correct += top[0].item() * pred.shape[0]
            n_total += pred.shape[0]
            dom_label = 0 if is_toybox else 1
            labels = dom_label * torch.ones(images.size(0)).cuda()
            top = utils.calc_binary_accuracy(output=dom_logits, target=labels)
            dom_correct += top.item() * pred.shape[0]
            dom_total += pred.shape[0]
        acc = n_correct / n_total
        dom_acc = dom_correct / dom_total
        return round(acc, 2), round(dom_acc, 2)

    def dom_eval(self, src_loader, trgt_loader):
        """Evaluate the model on the provided dataloader"""
        n_total = 0
        n_correct = 0
        self.network.set_eval()
        for i, loader in enumerate([src_loader, trgt_loader]):
            for _, (idxs, images, _) in enumerate(loader):
                labels = i * torch.ones(images.size(0))
                images, labels = images.cuda(), labels.cuda()
                with torch.no_grad():
                    _, _, dom_logits = self.network.forward(images)
                top, pred = utils.calc_binary_accuracy(output=dom_logits, target=labels)
                n_correct += top[0].item() * pred.shape[0]
                n_total += pred.shape[0]
        acc = n_correct / n_total
        return round(acc, 2)
