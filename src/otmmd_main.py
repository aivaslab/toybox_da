"""Main methods for the OT_MMD algorithm"""
import argparse
import csv
...
import datasets
import models_da
import datasets_otmmd
import models_otmmd
import networks_da
import otmmd_utils
import utils

TEMP_DIR = "../temp/guided_mmd/"
OUT_DIR = "../out/guided_mmd/"
...
def get_parser():
    """Return parser for OT-MMD experiments"""
    ...
    parser.add_argument("--asymmetric", default=False, action='store_true', help="Use this flag to use asymmetric "
                                                                                 "MMD loss")
    parser.add_argument("--mmdw", "-mmdw", default=0.1, type=float, help="Relative weight of mmd loss")
    parser.add_argument("--cld", "-cld", choices=["p", "ot"])
...
def main():
    ...
    if cld_type == "ot":
        cl_dict_loader = otmmd_utils.OT_CL_Dict(backbone_weights=net.backbone.model.state_dict())
    else:
        cl_dict_loader = otmmd_utils.Perfect_CL_Dict()
    cl_dict = cl_dict_loader.get_cl_dict()
    ot_mmd_data = datasets_otmmd.Toybox_IN12_OT(in12_cl_dict=cl_dict)
    ot_mmd_loader = torchdata.DataLoader(ot_mmd_data, batch_size=b_size, shuffle=True, num_workers=8, drop_last=True,
                                         pin_memory=True, persistent_workers=True, prefetch_factor=2)

    model = models_otmmd.OTMMDModel(network=net, loader=ot_mmd_loader, logger=logger, combined_batch=combined_batch,
                                    no_save=no_save, asymmetric=asymmetric, mmd_w=mmdw)
