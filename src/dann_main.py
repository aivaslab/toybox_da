"""Main methods for the DANN algorithm"""
import argparse
import csv
...
def get_train_test_acc(model, loaders, writer: tb.SummaryWriter, step: int, logger, no_save):
    ...
    for loader_name, loader in loaders.items():
        acc, dom_acc = model.eval(loader=loader, is_toybox="tb" in loader_name)
        accs[loader_name] = acc
        log_str += f"{loader_name} acc: {acc}  ({dom_acc}) "
...
def get_parser():
    ...
    parser.add_argument("--bblr", default=1.0, type=float, help="Weight of the backbone lr")
...
def eval_model(exp_args):
    ...
    assert load_file['type'] == networks_da.ResNet18DANN.__name__
    ...
    bb_wts, dom_cl_wts, cl_wts = (load_file['backbone'], load_file['dom_classifier'], load_file['classifier'])
    net = networks_da.ResNet18DANN(num_classes=12, backbone_weights=bb_wts, dom_classifier_weights=dom_cl_wts,
                                   classifier_weights=cl_wts)

    model = models_da.DANNModel(network=net, source_loader=src_loader_train, target_loader=trgt_loader_train,
                                logger=logger, no_save=True)
...
def main():
    ...
    if not no_save:
        os.makedirs(tb_path, exist_ok=False)
    ...
    if exp_args['load_path'] != "" and os.path.isdir(exp_args['load_path']):
        ...
        if "ResNet18DANN" in load_file['type']:
            logger.info(f"Loading backbone, classifier, dom_cl weights from {load_file_path} ({load_file['type']})")
            dom_cl_wts = load_file['dom_classifier'] if 'bottleneck' in load_file.keys() else None
            cl_wts = load_file['classifier'] if 'classifier' in load_file.keys() else None
        else:
            logger.info(f"Loading backbone weights from {load_file_path} ({load_file['type']})")
            dom_cl_wts, cl_wts = None, None
        net = networks_da.ResNet18DANN(num_classes=12, backbone_weights=bb_wts, dom_classifier_weights=dom_cl_wts,
                                       classifier_weights=cl_wts)
    else:
        if exp_args['pretrained']:
            logger.info(f"Loading model weights from ImageNet pretraining...")
        net = networks_da.ResNet18DANN(num_classes=12, pretrained=exp_args['pretrained'])

    model = models_da.DANNModel(network=net, source_loader=src_loader_train, target_loader=trgt_loader_train,
                                logger=logger, no_save=no_save)

    bb_lr_wt = exp_args["bblr"]

    optimizer = torch.optim.SGD(net.backbone.parameters(), lr=bb_lr_wt * exp_args['lr'], weight_decay=exp_args['wd'],
                                momentum=0.9, nesterov=True)
    optimizer.add_param_group({'params': net.dom_classifier_head.parameters(), 'lr': exp_args['lr'], 'wd': exp_args['wd']})
