...
def gen_plot(settings, out_path, fnames, markers, csv_data_lists, out_name):
    ...
    for idx, setting in enumerate(settings):
        ...
        fig, ax = plt.subplots(1, dpi=dpi, figsize=(80, 50))
        for i3, fname in enumerate(fnames):
            ...
            labels.append(label)
            fp.close()
        gen_one_plot(ax=ax, embeddings=embeddings, labels=labels, markers=markers)
        ax.set_title(f"nbr: {nbr}  d: {d}  m: {distance_metric}")
        ...
        fig.savefig(out_path + str(nbr) + "_" + str(d) + "_" + str(distance_metric) + ".png",
...
if __name__ == "__main__":
    ...
    gen_plot(settings=chosen_settings, out_path=dir_path,
             fnames=["tb_train.csv", "in12_train.csv"], markers=[".", "+"],
             csv_data_lists=[toybox_csv_data_train, in12_csv_data_train],
