...
class JANModelWithScrambledTargetClasses:
    """Module implementing the combined supervised pretraining on source and target"""
    ...
    def train(self, optimizer, scheduler, steps, ep, ep_total, writer: tb.SummaryWriter):
        ...
        for step in range(1, steps + 1):
            ...
            if self.combined_batch:
                comb_images = torch.concat([src_images, trgt_images], dim=0)
                src_feats, trgt_feats, src_logits, trgt_logits = self.network.forward(comb_images)
            else:
            ...
            src_loss = criterion(src_logits, src_labels)
            trgt_loss = criterion(trgt_logits, scrambled_trgt_labels)
            jmmd_loss = self.jmmd_loss((src_feats, func.softmax(src_logits, dim=1)),
                                       (trgt_feats, func.softmax(trgt_logits, dim=1)))
...
class JANModelWithScrambledTargetClassesV2:
    ...
    def train(self, optimizer, scheduler, steps, ep, ep_total, writer: tb.SummaryWriter):
        ...
        for step in range(1, steps + 1):
            ...
            comb_images = torch.concat([src_images, trgt_images], dim=0)
            src_feats, trgt_feats, src_logits_1, trgt_logits_1, \
                src_logits_2, trgt_logits_2 = self.network.forward(comb_images)
            
            scrambled_trgt_labels = self.scrambler.scramble(trgt_labels)
            
            src_loss = criterion(src_logits_1, src_labels)
            trgt_loss = criterion(trgt_logits_2, scrambled_trgt_labels)
            jmmd_loss = self.jmmd_loss(
                (src_feats, func.softmax(src_logits_1, dim=1), func.softmax(trgt_logits_1, dim=1)),
                (trgt_feats, func.softmax(src_logits_2, dim=1), func.softmax(trgt_logits_2, dim=1))
            )
            total_loss = src_loss + trgt_loss + alfa * jmmd_loss
